        ...
        NetService.uploadInput(
          ...
          (x) => {
            ...
            this.backdata.forEach((i, index) => {
              ...
              // compute persentage
              if (inputLabelAll != 0) {
                this.inputLabels["input"][0]["label"] = Math.round(
                  this.inputLabels["input"][0]["label"] / inputLabelAll
                );
                this.inputLabels["input"][1]["label"] = Math.round(
                  this.inputLabels["input"][1]["label"] / inputLabelAll
                );
                this.inputLabels["input"][2]["label"] = Math.round(
                  this.inputLabels["input"][2]["label"] / inputLabelAll
                );
                this.inputLabels["input"][3]["label"] = Math.round(
                  this.inputLabels["input"][3]["label"] / inputLabelAll
                );
                this.inputLabels["input"][4]["label"] = Math.round(
              ...
              if (i["elo_info"][2].length == 0) {
                ...
                this.highlightWords.push(
                  i["content"].substring(startTemp, endTemp + 1)
                );
              }
            ...
            for (var i = 0; i < this.inputS.length; i++) {
              ...
              labelSum.forEach((e, ind) => {
                labelSum[ind] += parseInt(
                  this.eachSentenceLabel[i.toString()][ind]
                );
              });
            }
            ...
            this.resultSentence.forEach((e, ind) => {
              ...
              if (e.length > 1) {
                ...
                if (this.eachSentenceLabel[ind][5] != 0) {
                  //claim
                  var strTemp =
                    '<span style="background-color: #90ee8f; ">' +
                    this.resultSentence[ind] +
                    "</span>"; //+ ".";
                  ...
                  //premise
                  var strTemp =
                    '<span style="background-color: #ffd701;">' +
                    this.resultSentence[ind] +
                    "</span>"; //+ ".";
                  //var claimType = this.eachSentenceLabel[ind][5]
                  eachSentenceHighlight = strTemp;
            ...
            this.highlightWords.forEach((err) => {
              ...
              this.allHighlightTxt = this.allHighlightTxt.replace(
                err,
